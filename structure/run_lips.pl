#!/share/opt/perl/5.18.1/bin/perl
##/usr/bin/perl -w 
###############################################################################
...
$pdb_id='/nethome/mct30/aphid/8971';
@helix_starts=();
@helix_ends=();
if(-e $spanfile)
{
...
# $blastout=$seq;
# $blastout="seq.out";
$blastout='/nethome/mct30/aphid/acypi8971.blast';

if(!-e $blastout)
{
    $psiblast_command="$PSIBLAST -i $seq -d $DB -j 2 -h 0.001 -b5000 -v5000 -o $blastout";
    # $psiblast_command="$PSIBLAST -query $seq -db $DB -num_iterations 2 -evalue 0.001 -num_alignments 5000 -num_descriptions 5000 -o $blastout";
    print "Running:\n $psiblast_command\n";
    `$psiblast_command`;
}
